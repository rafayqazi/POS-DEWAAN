                ...
                <tr class="hover:bg-gray-50/50 transition product-row" data-category="<?= strtolower($p['category']) ?>" data-unit="<?= strtolower($p['unit']) ?>">
                    ...
                    <td class="p-5">
                        <div class="flex justify-center gap-2">
                            <a href="quick_restock.php?open=<?= $p['id'] ?>" class="restock-btn w-8 h-8 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-600 hover:text-white transition flex items-center justify-center shadow-sm" title="Restock"><i class="fas fa-plus text-[10px]"></i></a>
...
<!-- 1. Add / Edit Product Modal (Dual Mode) -->
<div id="addProductModal" class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden">
        <div class="p-6 border-b flex justify-between items-center bg-gray-50/50">
            <h3 id="addProductModalTitle" class="text-xl font-black text-gray-800 uppercase tracking-tight">Add New Product</h3>
        ...
        <div class="p-8 overflow-y-auto flex-1 custom-scrollbar">
            <form id="addProductForm" method="POST" class="space-y-6">
                <input type="hidden" name="action" id="addProductAction" value="add">
                <input type="hidden" name="id" id="addProductId" value="">
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Product Name</label>
                        <input type="text" name="name" id="add_name" required class="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500 transition font-bold" placeholder="Enter product name...">
                    </div>
                    <div class="col-span-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Description</label>
                        <textarea name="description" id="add_description" class="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500 transition text-sm" placeholder="Optional product description..."></textarea>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <input type="number" step="any" name="sell_price" id="add_sell_price" required class="p-3 border rounded-xl font-bold" placeholder="Sell Price">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div id="add_qty_row">
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1" id="add_qty_label">Initial Stock</label>
                    ...
                    <div id="add_purchase_date_row">
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Purchase Date</label>
                        <input type="date" name="purchase_date" id="add_purchase_date" value="<?= date('Y-m-d') ?>" class="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500 transition">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Expiry Date</label>
                        <input type="date" name="expiry_date" id="add_expiry_date" class="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500 transition">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Remarks</label>
                        <input type="text" name="remarks" id="add_remarks" class="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500 transition" placeholder="Optional notes...">
                    </div>
                </div>
                <!-- Dealer section: only shown for Add mode -->
                <div id="add_dealer_section" class="bg-gray-50 p-5 rounded-2xl border border-gray-100 space-y-4">
                    ...
                    <div id="add_calc_summary" class="hidden space-y-2 p-4 bg-teal-50/50 rounded-xl border border-teal-100/50">
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Current Bill:</span><span id="add_summary_bill" class="font-bold text-gray-800">Rs. 0</span></div>
                        <div class="flex justify-between text-xs"><span class="text-gray-500">Dealer Balance:</span><span id="add_summary_bal" class="font-bold text-gray-800">Rs. 0</span></div>
                        <div class="pt-2 border-t border-teal-100 flex justify-between text-sm"><span class="font-bold text-teal-800 uppercase text-[10px]">Projected Balance:</span><span id="add_summary_new_bal" class="font-black text-teal-700">Rs. 0</span></div>
                    </div>
                    <div>
                ...
                <button type="submit" id="addProductSubmitBtn" class="w-full py-4 bg-teal-600 text-white rounded-2xl font-bold">Save Product</button>
            </form>
        </div>
    </div>
</div>

<!-- 3. Restock Modal (legacy/fallback, kept for direct inventory restocks) -->
...
// Open addProductModal in EDIT mode, pre-filled with existing product data
function openEditInAddModal(p) {
    // Switch to edit mode
    document.getElementById('addProductAction').value = 'edit';
    document.getElementById('addProductId').value = p.id;
    document.getElementById('addProductModalTitle').innerText = 'Edit Product';
    document.getElementById('addProductSubmitBtn').innerText = 'Update Product';
    document.getElementById('addProductSubmitBtn').className = 'w-full py-4 bg-blue-600 text-white rounded-2xl font-bold';
    
    // Hide purchase date & dealer section (not needed for edit)
    document.getElementById('add_purchase_date_row').classList.add('hidden');
    document.getElementById('add_dealer_section').classList.add('hidden');
    document.getElementById('add_qty_label').innerText = 'Current Stock';
    
    // Fill fields
    document.getElementById('add_name').value = p.name || '';
    document.getElementById('add_description').value = p.description || '';
    document.getElementById('add_category').value = p.category || '';
    document.getElementById('add_unit_select').value = p.unit || '';
    document.getElementById('add_buy_price').value = p.buy_price || '';
    document.getElementById('add_sell_price').value = p.sell_price || '';
    document.getElementById('add_expiry_date').value = p.expiry_date || '';
    document.getElementById('add_remarks').value = p.remarks || '';
    
    // Calculate display stock qty (convert base units back to primary unit)
    const h = getUnitHierarchyJS(p.unit);
    let f2 = parseFloat(p.factor_level2) || 1;
    let f3 = parseFloat(p.factor_level3) || 1;
    let m = 1;
    if(h.length > 2) m = f2 * f3;
    else if(h.length > 1) m = f2;
    const displayQty = (parseFloat(p.stock_quantity) / m).toFixed(2);
    document.getElementById('add_stock_qty').value = displayQty;
    
    // Set factor hidden inputs before triggering UI update
    document.getElementById('add_f2').value = f2;
    document.getElementById('add_f3').value = f3;
    updateFactorUI('add', { q: displayQty, f2, f3 });
...
window.closeModal = function(id) {
    if(id === 'addProductModal') {
        ...
        document.getElementById('addProductSubmitBtn').innerText = 'Save Product';
        document.getElementById('addProductSubmitBtn').className = 'w-full py-4 bg-teal-600 text-white rounded-2xl font-bold';
        document.getElementById('add_purchase_date_row').classList.remove('hidden');
        document.getElementById('add_dealer_section').classList.remove('hidden');
        document.getElementById('add_qty_label').innerText = 'Initial Stock';
    }
    if(id==='addProductModal' && window.needsReload) location.reload();
    else document.getElementById(id).classList.add('hidden');
};

document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.edit-btn').forEach(b => b.addEventListener('click', () => openEditInAddModal(JSON.parse(b.dataset.product))));
...
<div style="padding:40px; font-family:'Segoe UI',Arial,sans-serif; color:#222; max-width:900px; margin:0 auto;">
    ...
    <div style="margin-bottom:35px;">
        <h4 style="font-size:11px; text-transform:uppercase; letter-spacing:3px; color:#888; margin:0 0 10px;">Category Index</h4>
        <table style="width:100%; border-collapse:collapse; font-size:12px;">
            <thead><tr style="background:#0d9488; color:#fff;"><th style="padding:8px 12px; text-align:left; width:40px;">#</th><th style="padding:8px 12px; text-align:left;">Category</th><th style="padding:8px 12px; text-align:center; width:130px;">Total Products</th></tr></thead>
            <tbody>
                <?php $idx=1; foreach($g_cat as $cat_name=>$cat_items): ?>
                <tr style="background:<?= $idx%2===0?'#f9fafb':'#fff' ?>; border-bottom:1px solid #eee;">
                    <td style="padding:7px 12px; color:#aaa; font-size:11px;"><?= $idx++ ?></td>
                    <td style="padding:7px 12px; font-weight:600; color:#0d9488; text-transform:uppercase; font-size:11px; letter-spacing:0.5px;"><?= htmlspecialchars($cat_name) ?></td>
                    <td style="padding:7px 12px; text-align:center; font-weight:bold; color:#333;"><?= count($cat_items) ?></td>
                </tr>
                <?php endforeach; ?>
    ...
    <?php foreach($g_cat as $cat_name=>$cat_items): usort($cat_items, function($a,$b){return strcmp($a['name'],$b['name']);}); ?>
    <div style="margin-bottom:35px;">
        <h2 style="color:#0d9488; border-bottom:2px solid #e5f7f5; padding-bottom:8px; margin-bottom:15px; text-transform:uppercase; font-size:13px; letter-spacing:2px;">
            <?= htmlspecialchars($cat_name) ?> <span style="font-size:10px; font-weight:normal; color:#aaa; margin-left:8px;">(<?= count($cat_items) ?> items)</span>
        </h2>
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
            <?php foreach($cat_items as $item): ?>
            <div style="display:flex; justify-content:space-between; align-items:flex-start; border-bottom:1px dashed #eee; padding-bottom:8px; gap:10px;">
                <div style="flex:1;"><div style="font-weight:bold; color:#222; font-size:12px;"><?= htmlspecialchars($item['name']) ?></div><?php if(!empty($item['description'])): ?><div style="font-size:10px; color:#888; margin-top:2px;"><?= htmlspecialchars($item['description']) ?></div><?php endif; ?></div>
                <div style="font-weight:900; color:#0d9488; white-space:nowrap; font-size:13px;"><?= formatCurrency($item['sell_price']) ?></div>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
    <?php endforeach; ?>
    <div style="margin-top:50px; border-top:1px solid #e5e7eb; padding-top:18px; text-align:center;">
        <p style="margin:0; font-size:9px; color:#bbb; text-transform:uppercase; letter-spacing:2px;">Software Developed By</p>
        <p style="margin:4px 0 0; font-weight:bold; font-size:12px; color:#555;">Abdul Rafay</p>
        <p style="margin:2px 0 0; font-size:10px; color:#aaa;">WhatsApp: 03000358189 / 03710273699</p>
    </div>
</div>
</div>
