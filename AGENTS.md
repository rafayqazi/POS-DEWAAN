...
- **Check Inventory (`pages/check_inventory.php` + print report):** IN/OUT columns = restocks / net sales **inside the selected date range** (quantities converted to base units). **Final Stock = all restock IN with date ≤ Date To − all net sales with date ≤ Date To** (so selling older stock does not fake a negative just because those restocks sit outside `from`). Opening qty typed at product create is the first restock on `products.created_at` (written by inventory add); Stock Logs labels that row **Opening** and shows a running **Balance After**. Do not back-calculate Final from `products.stock_quantity`. Summary cards for value / units in hand still use current `stock_quantity`. Print report (`print_inventory_report.php`) still uses period IN−OUT unless updated separately.
