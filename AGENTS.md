...
│   └── process_return.php, restock_process.php, save_expense.php, reconcile_stock.php, ...
...
- **Check Inventory (`pages/check_inventory.php` + print report):** IN/OUT columns = restocks / net sales **inside the selected date range** (quantities converted to base units). **Final Stock = live `products.stock_quantity` (same field as POS availability), rolled back by restocks/sales after Date To** so Lifetime / Date To=today matches POS. Period IN−OUT can differ from Final when stock was changed without a restock/sale log (inventory qty edit, etc.). Opening qty typed at product create is the first restock on `products.created_at` (written by inventory add); Stock Logs labels that row **Opening** and shows a running **Balance After**. Summary cards for value / units in hand also use current `stock_quantity`.
